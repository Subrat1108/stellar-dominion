...
describe("FoundColony", () => {
  it("conserves supplies — seeds the colony by drawing from the ship (no-tile founding = neutral site modifiers + EDL only)", () => {
    ...
    // foundAt founds WITHOUT a tile (headless), so there is no per-tile volatile
    // head-start and no tile setup cost — only the body-level EDL setup cost
    // applies. (Tile-driven modifiers are covered by the surface-layer tests.)
    const pid = planetId(world);
    const body = world.components.celestialBody.get(pid)!;
    const edlCost = edlSetupCost(landingViability(body));

    const colony = foundAt(world, pid);

    // Ship: metals = seed + EDL setup; food/fuel = seed; life-support debit unchanged.
    expect(inv.metals).toBe(m0 - COLONY_SEED.metals - edlCost);
    ...
    // Colony: metals = seed; water/oxygen = seed (no in-situ head-start without a tile).
