...
### The landing arc — making landing a meaningful choice *(active)*
The first slice of the fun loop proper (`docs/15` §1: EXPLORE → **LAND** → COLONISE → …). Landing stops being a bare "you are now on the surface" transition and becomes *a choice that shapes the colony you found*. Reference: `docs/14-landing-and-surface.md` (aggregate globe, NO tile/AP/rover layer; site science → scalar modifiers). Pieces, built in order:
- **Owner-scoped colonies + save migration** — the first player-facing application of the hard multi-agent rule (`docs/15` §6): colonies carry an `ownerId`, the local player is AN owner (`localOwnerId`), and every founding/build/terraform mutation rides the command layer with an **actor envelope** (`{ command, actorId }`, `applyCommand(world, cmd, actorId)`) so AI/multiplayer owners slot in with no retrofit. This changes the colony shape → **save v2→v3**, migrated (not broken) via a reusable migrator chain (existing colonies → the local-player owner). The dress rehearsal for the production save-migration discipline every future save change needs.
- **Candidate landing sites** — on landing, deterministically generate ~3 candidate sites from the body seed (each a bundle of `docs/14` attributes: latitude/insolation, volatile/water proximity, slope, radiation/hazard, thermal inertia). Options to weigh, not a tile map; regenerated on demand, never persisted.
- **Site → founding modifiers** — selecting a site founds the colony with starting modifiers derived by a pure `siteModifiers()` mapping applied to the existing aggregate colony (volatile proximity → water/oxygen head-start; insolation → persistent solar efficiency; radiation → shielding cost; slope → setup cost).
- **Minimal EDL** — a pure `landingViability(body)` classifier (vacuum/thin/nominal/thick from pressure + gravity) shown as a landing affordance, with one light hook (payloadFactor nudges setup cost). Not a descent sim; the most deferrable piece.
- **Interdependence seed** — Fissiles introduced as the first Strategic-Core resource (`docs/16`), **presence/absence only**, surfaced legibly ("no local Fissiles → depends on imports") to create the pull toward other worlds with zero economy machinery.

**Out of scope / deferred:** the strategic-resource network / trade routes / logistics / transport (the economy slice, `docs/16`); rival/AI owners; 3B depth; any tile/AP/rover/surface-spatial layer (never); the hard-start retune; a real EDL descent/risk mechanic.
...
- **Dedicated economy / anti-snowball pass** — admin latency, gravity-well tax, bulk-vs-strategic resource split (`docs/12`). Reference brief: `docs/16-economy-and-interdependence.md` (two-tier local-bulk/strategic resource split, virtual trade routes + distance trade-loss, the anti-snowball dependency trio, terraforming as a SOFT strategic sink, owner-scoped per the `docs/15` multi-agent seam).
