> **⚠ SUPERSEDED IN PART BY `docs/17` (2026-07-20).** The "NO tile layer, ever" stance below was reversed by a deliberate pillar change: the planet now HAS a 2D tile surface you land on and place colonies on (`docs/17`, `docs/09` 2026-07-20). The **load-bearing boundary survives**: tiles are terrain + placement + site-modifiers; the colony ECONOMY stays aggregate; tiles are NOT a per-tile economy sim. This doc's **site-science → founding-modifiers** design (§2) is preserved verbatim — the tile just replaces the card as the source of those attributes. Read §2's `siteModifiers()` mapping as current; read the "never a spatial map" clauses as historical.
...
> (1) ~~**NO hex/tile/action-point surface layer, ever.**~~ **(Reversed — see `docs/17`.)** The tile SURFACE now exists; what remains true is the deeper intent: no per-tile economy sim, no AP/build-radius/rover micro. The colony stays an **aggregate model** (`docs/10`); tiles feed it scalar MODIFIERS + a placement location, not a second economy.
