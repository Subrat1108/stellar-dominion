 ...
 * the surface mode (App's landed gate → SurfaceMode) is driven by this ref,
 * which is normally only updated by the Landed/TookOff transition EVENTS
 * (main.ts) — but a save reload restores the sim STATE directly
 * (ctrl.landedBodyId) without ever firing those events, so the ref would
 * otherwise stay stale/null even though the ship is landed, leaving the player
 * stuck with no surface mode / TAKE OFF. Call this once right after
 * building/restoring a world and BEFORE the first UI render.
 */
