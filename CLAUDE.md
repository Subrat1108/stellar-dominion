...
- **Scope (Session 21):** cockpit + control model + steering + gravity + autopilot auto-throttle only. **Next:** confirm in-browser (steering feel + clean pointer-lock entry/exit that doesn't fight HUD clicks; autopilot→manual gravity handoff with no fall-in; body grows from the cockpit POV; trapezoidal arrival) — see the Session-21 devlog checklist — then **Exploration polish C** (the unified clickable multi-scale map) and **D** (body detail panel). **Out of scope / deferred:** the `docs/05` Deferred/parked list; a real-mass-derived μ (density-varied wells) and a dedicated ship near-camera layer remain noted tuning follow-ups.
- **Next action:** user in-browser confirmation of Polish B feel (checklist in `docs/07` Session 21), then **Exploration polish C**.
