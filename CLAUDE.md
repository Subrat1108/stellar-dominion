...
- **Phase:** 2C — **complete + legibility pass (Session 13).** Per-building operational status added to the colony sim and UI. Each economy tick, `colonySystem` computes `colony.buildingStatuses` (transient/derived): solar always `running`; POWER_PRIORITY buildings marked `idle-no-power` when power-shed; powered buildings with insufficient material input marked `idle-no-input` with the limiting resource named. **ColonyPanel** shows a colour-coded status line under each built structure (green running / amber idle: reason) and tags bottleneck resources in the ledger (amber "↑ needed"). **Legibility** added as Design Pillar 6 in `docs/01` — governs Phase 3 terraforming levers (will show locked with unmet prerequisite named). **88/88 tests; typecheck clean.**
- **Confirmed (Session 13):** `idle-no-input` state only set when `canRun === total` (power is not the issue); partial-power case stays `idle-no-power` with running/total count shown; `limitingResource` is machine-readable for bottleneck tagging separate from the human reason string.
- **Prior (Session 12):** Aggregate population, housing capacity, proportional death rates, habitation power penalty, popLimitingFactor — all in `Colony`; `populationStep()` runs after flows each economy tick.
- **Next action:** confirm in-browser (found colony → build hydroponics before water → see "idle: insufficient water" → build water extractor → see "running"; confirm bottleneck tag on Water row disappears), then Phase 3 — terraforming loop.
