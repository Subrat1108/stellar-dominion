...
- **Phase:** 1B — **complete** (polished, Session 8). **Six-key flight:** W/S thrust, A/D yaw, ↑/↓ pitch (climb/dive by pitch-then-thrust; no world-vertical). Thrust drives along the nose; **yaw is now correct** (D = turn right). **Three camera views** — cockpit / chase / map; `C` cycles, `M` toggles map; right-drag looks around. **Floating origin** moves the whole system (star + planets + rings + starfield in one `worldRoot` group); ship stays at render (0,0,0). **Real-AU scale** (`presentation.ts`: `AU_TO_SCENE` 200) spreads the system out — Titan's Eye ~704 u, planet radii ~1 u so a planet reads as a point that grows on approach. **Speed = throttle** (ship `maxSpeed` 0.01 base): 1× = fine docking, 1000× crosses to Titan's Eye in ~1 min. **Orbital time decoupled** (`ORBITAL_TIME_RATE` 0.0015) so planets stay near-stationary during a flight. **System minimap** (top-left radar, click-to-course) + **proximity readout** (DebugPanel NEAREST row). **Orbit model is moon-ready** (`orbitalSystem` composes with `Orbit.parent`'s transform; no moons built yet). Autopilot steers 3D toward a "SET COURSE" target. 43/43 tests; typecheck + build clean.
- **Confirmed (Session 8):** yaw fix is sim-layer (Input contract says +1 = right); distance scale is real-AU with compressed render radii; minimap is system-scale only (galaxy zoom parked to Phase 5); orbit parent-composition is the moon-ready data path.
- **Next action:** confirm flight feel in-browser (yaw, approach growth, ~1 min cruise, minimap), then Phase 2 — first colony (land on a body, found a dome, resource flows).
