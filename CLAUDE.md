...
- **Phase:** 2C — **complete** (population dynamics, Session 12). **Aggregate population** added to `Colony` (float, rendered as floor). `FoundColony` seeds `population = crewCount` (5) and grants 1 free **Habitation Module** (10 housing capacity). Additional modules cost 80 Metals, 3 pw draw. Housing capacity = `buildings.habitation × 10`. **6th building** added to `BUILDING_TYPES` and `POWER_PRIORITY` (between hydroponics and smelter). Colony consumption now scales with `colony.population` (not hardcoded crew). **`populationStep()`** runs after flows each economy tick: oxygen critical (stockpile < 10 AND net < 0) → −1.5 %/tick; water → −1.0 %/tick; food → −0.4 %/tick (all require net < 0 — recovering stockpile ≠ shortage); housing full → no growth; otherwise grow at `GROWTH_RATE_BASE × habitability × housingPowerMultiplier` (0.5–1.0, unpowered habitation halves growth). **ColonyPanel** shows POPULATION count/capacity/rate/limiting-factor. **Known debt:** founding crew counted as both ship crew and colony population — reconcile in Phase 5–6. 82/82 tests; typecheck + build clean.
- **Confirmed (Session 12):** proportional death rates; food net < 0 required for starvation; habitation power draw penalises growth (×0.5–1.0 multiplier); aggregate population (not Pops); founding crew double-counted (cosmetic debt).
- **Next action:** confirm in-browser (land Mira → found → pop = 5, housing = 10 → build solar+water+electrolysis → pop grows → cut O₂ → rapid decline visible), then Phase 3 — terraforming loop.
