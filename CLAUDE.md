...
- **Confirmed (Session 11):** the 6-resource set is final/data-driven (Propellant accumulates, sink in Phase 4); economy is decoupled from the flight tick via one constant; founding conserves consumables (nothing from thin air); supplier-before-consumer power priority avoids a water death-spiral; multiplayer stays deferred but determinism/serialisation are preserved for a possible future lockstep.
- **Next action:** confirm the loop in-browser (land Mira → found → build solar+water+electrolysis → O₂ net-positive → survival clock recovers), then Phase 2C — population dynamics.
